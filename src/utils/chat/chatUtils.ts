...
// Check if a user is online (safe if activeUsers is undefined/null)
export const isUserOnline = (
  userId: string,
  activeUsers: ChatUser[] | undefined | null
): boolean => {
  if (!activeUsers || !Array.isArray(activeUsers)) return false;
  return activeUsers.some((u) => u.uid === userId);
};

// ✅ Filter users by name or email, substring + case-insensitive
